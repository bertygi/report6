...
public class Sudoku extends Box{
    ...
    /**
     * コンストラクタ。
     * 9より大きい数字や、0より小さい数字が入力された場合は、エラーメッセージを出力する。
     */
     ...
     * 結果を出力するメソッド。
     */
    public void resultPirnt() {
        int n = 0;
        int m = 0;
        Memo result = new Memo();
        int[][][] resultSudoku = result.getResult();
        for (int k = 0; k<3; k++) {
            for (int j = 0; j<3; j++){
                for (int i = n; i<n+3; i++) {  
                    System.out.print(Arrays.toString(resultSudoku[i][j]));
                    if ((i+1)%3 == 0) {
                        System.out.println();
                    }
                }
    ...
    public void solveSudoku() {
        ...
        do{
            ...
            for(int i=0; i<9; i++) {
                for (int j=0; j<3; j++) {
                    for (int k =0; k<3; k++) {
                        copyResult1 += solve.getResult()[i][j][k];
                    }
                }
            }
            solve.solve(sudoku);
            for(int i=0; i<9; i++) {
                for (int j=0; j<3; j++) {
                    for (int k =0; k<3; k++) {
                        copyResult2 += solve.getResult()[i][j][k];
                    }
                }
            }
        }while(copyResult1 != copyResult2);
        if(copyResult1 == 405) {
            resultPirnt();
        }else {
            System.out.println("すみません。今のバージョンでは解けない数独です。アップデートを待ってください。");
        }
    }
